// ============================================================
// CATEGORY-PAGE.JS — Logic Halaman Project (Macro/Mod Game)
...
(function () {
  ...
  function getProductVideos(product) {
    return [
      { slot: 1, source: pickVideoSource(product.videoUrl, product.videoId),
        label: product.video1Label || "Cek Video", thumbnail: product.video1Thumbnail || product.image },
      { slot: 2, source: pickVideoSource(product.videoUrl2, product.videoId2),
        label: product.video2Label || "Cek Video", thumbnail: product.video2Thumbnail || product.image }
    ].map(item => {
      const video = window.BAM.normalizeVideo(item.source);
      if (!video) return null;
      return { ...item, video, thumbnail: window.BAM.videoThumb(item.source, item.thumbnail) };
    }).filter(Boolean);
  }
  ...
  // ── AMBIL VIDEO DARI SEMUA PRODUK ──

  function getAllProductVideos(category) {
    const products = category.products || [];
    const videos = [];
    products.forEach(product => {
      const sources = [
        { source: pickVideoSource(product.videoUrl, product.videoId), label: product.video1Label || product.name, desc: "" },
        { source: pickVideoSource(product.videoUrl2, product.videoId2), label: product.video2Label || product.name, desc: "" }
      ];
      sources.forEach(s => {
        const n = window.BAM.normalizeVideo(s.source);
        if (n) {
          videos.push({
            normalized: n,
            thumbnail: window.BAM.videoThumb(s.source, product.image),
            title: s.label,
            description: s.desc
          });
        }
  ...
  function renderInfoUpdate(category) {
    ...
    const productVideos = getAllProductVideos(category);
    target.innerHTML = [
      renderNews(info.news, info.badge),
      productVideos.length ? renderVideoPanel("Video Produk", productVideos) : ""
    ].join("");
  }
  ...
  function openProductVideo(slot = 1) {
    ...
    if (video) { openVideoUrl(video.video.url); return; }
  ...
  function bindCategoryEvents() {
    document.addEventListener("click", event => {
      ...
      if (productVideo) { openProductVideo(Number(productVideo.dataset.productVideo)); return; }
      if (event.target.closest("[data-active-product-video]")) { openProductVideo(1); }
    });
  }
